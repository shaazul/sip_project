...
export class LayoutService {
    ...
    constructor() {
        setTimeout(() => {
            // Load persisted config on service creation
            const saved = this.getPersistedConfig();
            if (saved) {
    ...
    private getPersistedConfig(): layoutConfig | null {
        try {
            const raw: any = localStorage.getItem(this.storageKey);
            const rawData = JSON.parse(raw);
            rawData.menuMode = 'overlay';
            return raw ? (rawData as layoutConfig) : null;
        } catch {
            return null;
        }
    }
